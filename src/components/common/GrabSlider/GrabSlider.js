...
const GrabSlider = (props) => {
  ...
  useEffect(() => {
    ...
    if (mounted) {
      if (scrollingDiv.current.scrollWidth <= scrollingDiv.current.clientWidth) {
        setRightButtonVisible(false);
      } else {
        setRightButtonVisible(true);
      }
    ...
    <div className={`${styles.tag_container_outer} hide-on-fullscreen`}>
      ...
      >
        {props.testTags?.map((tag) => {
          return <span className={styles.scroll_child} key={tag}>{`#${tag}`}</span>;
        })}
      </div>
      <div
      ...
      >
        <button
          className={`${styles.button} ${rightButtonVisible ? styles.visible : styles.invisible}`}
